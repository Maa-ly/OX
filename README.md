...
## Nautilus server — Quick setup & critical commands

The project contains a Nautilus Nitro Enclave server under `nautilus-server/`.
Below are the most important, copy-pasteable commands and notes pulled from
the `nautilus-server/README.md`. I am appending these here for quick access —
I did not change any existing documentation above.

Quick steps (enclave image build & run)

```bash
# from repo root
cd nautilus-server

# configure the enclave (defaults to the myanimelist app)
export KEY_PAIR=<your-key-pair>
./configure_enclave.sh myanimelist

# build the enclave image (replace MAL_CLIENT_ID / MAL_BEARER_TOKEN)
make ENCLAVE_APP=myanimelist MAL_CLIENT_ID=<your-myanimelist-client-id> MAL_BEARER_TOKEN=<your-myanimelist-bearer-token>

# run in debug mode (recommended for first runs)
make run-debug
...
Local dev (run Rust server without enclave)

```bash
# run the server locally (reads API_KEY from env)
cd nautilus-server/src/nautilus-server
export API_KEY=your_key_here   # bash / WSL
cargo run
```

Important runtime / troubleshooting notes

- If `run.sh` is permission-denied, make it executable and rebuild:
   ```bash
   chmod +x run.sh
   make clean
   make ENCLAVE_APP=myanimelist MAL_CLIENT_ID=<...> MAL_BEARER_TOKEN=<...>
   ```
- If you need WAL/Sui tooling for the broader project (optional):
   ```bash
   npm install -g pnpm
   suiup install walrus@testnet
   ~/.local/share/suiup/binaries/testnet/walrus --version
   ```
...
# nitro enclaves logs (host)
sudo cat /var/log/nitro_enclaves/err*.log
```

If you want, I can also add a small checklist of required env vars
(`MAL_CLIENT_ID`, `MAL_BEARER_TOKEN`, `API_KEY`) and where they're used.
